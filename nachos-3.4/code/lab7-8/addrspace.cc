...
//query whether there are some message in page
...
/*******Added to achieve the test*********/
	for (i = 0; i < numPages; i++){
                for (k = 0; k < PageSize; k++)
                        machine->mainMemory[ pageTable[i].physicalPage * PageSize + k ] = 0;
	}
...
/*******Added to achieve the test*********/
        int pAddr;// translate the logical address to physical address
                machine->Translate(noffH.code.virtualAddr, &pAddr, 4, TRUE);
/*******Added to achieve the test*********/
        executable->ReadAt(&(machine->mainMemory[pAddr]),//changed
...
/*******Added to achieve the test*********/
        int pAddr;
                machine->Translate(noffH.initData.virtualAddr, &pAddr, 4, TRUE);
/*******Added to achieve the test*********/
        executable->ReadAt(&(machine->mainMemory[pAddr]),//changed
...
/*******Added to achieve the test*********/
void AddrSpace::setPreAddrSpace(AddrSpace *pSpace)
{//set the precede address space
	preAddrSpace = pSpace;
}
void AddrSpace::setNextAddrSpace(AddrSpace *nSpace)
{// set the next address space
	nextAddrSpace = nSpace;
}
/*******Added to achieve the test*********/
...
//added a print method  to output the number of pages
